...
import { ScheduleOption } from '@/modules/schedule/schedule.option';
...
export class SetScheduleCommand {
	...
	async onSetSchedule(@Context() [interaction]: SlashCommandContext, @Options() { isNotify }: ScheduleOption) {
	...
	private sectionsToTable(sections: Section[], isNotify: boolean) {
		...
		if (isNotify) scheduleMessage.unshift(`<@&1046629026355228762>`);
	...
	private parseTextToSections(year: string, scheduleText: string): Section[] {
		...
		return lines
			...
			.map((matches) => {
				if (matches) {
					return {
						from: dayjs(`${year}.${matches[2]}`, 'YYYY.DD.MM').toDate(),
						to: dayjs(`${year}.${matches[3]}`, 'YYYY.DD.MM').toDate(),
