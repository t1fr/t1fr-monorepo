...
export class BudgetStage implements CalculateStage {
	...
	calculate(results: CalculateData[]): CalculateData[] {
		...
		//
		// let totalPoints = results.reduce((acc, val) => acc + val.currentPoints, 0);
		// if (this.budget >= totalPoints) return results;
		//
		// const groups = groupBy(results, (result) => result.currentPoints);
		// do {
		// 	for (const point in groups) {
		// 		groups[point].forEach((result) => {
		// 			if (result.currentPoints !== 0) {
		...
		// 	const increment = Math.min(quotient, maxPoint - groups[maxPoint][0].currentPoints);
		// 	if (increment === 0) break;
		// 	totalPoints += increment * recordCount;
		// 	groups[maxPoint].forEach((result) => {
		// 		result.currentPoints += increment;
