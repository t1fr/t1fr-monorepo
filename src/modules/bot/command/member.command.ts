...
import { PointTypes } from "@/modules/management/point/point.schema";
...
import { JoinOption, PointListOption } from "@/modules/bot/option/member.option";
...
export class MemberCommand {
	...
	async listPoint(@Context() [interaction]: SlashCommandContext, @Options() { type }: PointListOption) {
		...
		const embeds = [];
		const selections = ["nickname", ...typeSelection];
		const headers = ["暱稱", ...typeSelection]
		for (let i = 0; i < results.length; i += 25) {
			// @ts-ignore
			const fields = selectToFields(results.slice(i, i + 25), selections, headers );
