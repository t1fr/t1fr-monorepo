...
export class MemberUserCommand {
	...
	constructor(private readonly memberRepo: MemberRepo, private configService: BotConfigRepo) {}
		...
		if (member) {
			try {
				...
				await Promise.all([
					...
					isCore
						? rolesManager.add(this.coreRoleId, this.squadBattleRoleId)
						: rolesManager.remove(this.coreRoleId, this.squadBattleRoleId),
				]);
				isSuccess = true;
			} catch (e: any) {
