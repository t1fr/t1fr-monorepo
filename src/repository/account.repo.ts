...
export class AccountRepo implements OnModuleInit {
  ...
  constructor(private dynamicConfig: ConfigRepo, private prisma: PrismaService, private httpService: HttpService) {
  }
  ...
  public async fetchFromWeb(callback: ProgressCallback) {
    const html = await this.getHtml(this.squadAccountListUrl);
    if (!html) {
      return;
    }
    ...
    const cellValues = $('.squadrons-members__grid-item')
      ...
      .map((element, index) =>
        (index % 6 === 1)
          ? $(element).children('a').attr('href')!!.trim()
          : $(element).text().trim(),
      );
    ...
    for (let i = 0; i < cellValues.length; i += columnsCount) {
      ...
      const id = row[1].replace('en/community/userinfo/?nick=', '').replace('@psn', '').replace('@live', '') ?? '未知';
      const personalRating = row[2];
      const activity = row[3];
      const role = row[4];
      const joinDate = moment(row[5], 'DD.MM.YYYY').toDate();
      inputs.push({
        num: num,
        id: id,
        personalRating: parseInt(personalRating),
  ...
  async joinOnId() {
    return this.prisma.$queryRaw<OwnershipData[]>
