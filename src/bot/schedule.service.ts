...
import { BooleanOption, Context, Ctx, Modal, ModalContext, ModalParam, Options, SlashCommand, SlashCommandContext } from 'necord';
...
import { SectionRepo } from '../repository/section.repo';

class SetScheduleOptions {
  @BooleanOption({
    name: 'is-notify',
...
export class SetScheduleService {
  constructor(private memberRepo: MemberRepo, private dynamicConfigService: ConfigRepo, private sectionRepo: SectionRepo) {
  }
  ...
  async onSetSchedule(@Context() [interaction]: SlashCommandContext, @Options() { isNotify }: SetScheduleOptions) {
  ...
  private parseTextToSections(year: string, scheduleText: string): Section[] {
    ...
    return lines.map(line => line.match(/(\d*\.\d*)\s*\((\d*\.\d*).*?(\d*\.\d*)\)/))
      .map(matches => matches ? new Section(
