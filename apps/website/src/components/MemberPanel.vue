...
<template>
    <div class="relative flex align-items-center">
        <Dialog v-model:visible="isDialogVisible" modal class="w-25rem" content-class="flex gap-2">
            <template #header>
                <div class="flex align-items-center">
                    <i :class="PrimeIcons.EXCLAMATION_TRIANGLE" class="mr-3" />
        ...
        <template v-if="userData">
            ...
            <Button label="登出" icon-pos="right" :icon="PrimeIcons.SIGN_OUT" text @click="onLogoutClick" />
        </template>
        <template v-else>
            <Button label="登入" icon-pos="right" :icon="PrimeIcons.SIGN_IN" text @click="authStore.startOAuth()" />
        </template>
    </div>
</template>
